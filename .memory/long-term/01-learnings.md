...
### 구현 결과
**완료된 기능**:
1. 원자적 파일 쓰기 (임시파일 + rename)
2. 스트리밍 체크섬 계산 (ChecksumWriter)
3. 실패 시 자동 정리 (defer)
4. Mock 파일시스템으로 테스트 커버리지 확보
...
1. Red: 백업 중단 시나리오 테스트 작성
2. Green: 최소 코드로 테스트 통과
3. Refactor: ChecksumWriter 추출, 인터페이스 정리
...
### 필요성 인식
**발견**: 백업/복원 등 장시간 작업에서 진행률 필수
**문제**: 터미널, CI/CD, 로그, TUI 각각 다른 방식 필요
**해결**: 하나의 구현으로 모든 환경 지원

### 핵심 설계 결정
1. **3초 규칙**: 3초 이상 작업은 진행률 필수
2. **다중 출력**: 터미널(\r), 로그(10초), TUI(채널)
3. **io.TeeReader**: 스트리밍 중 진행률 추적
4. **환경 감지**: term.IsTerminal() 활용

### 구현 통찰
**단순함의 힘**:
```go
// 이게 전부!
pw := &progressWriter{current: 0, total: size}
reader := io.TeeReader(source, pw)
io.Copy(dest, reader)
```

**복잡도**: +5/100 (추가 라이브러리 없음)

### 교훈
- 표준 라이브러리로 충분한 경우가 많음
- 환경별 분기보다 통합 접근이 유리
- 사용자 경험은 작은 디테일에서 결정됨
- 진행률 표시는 선택이 아닌 필수
...
# 동시에 로그에
[INFO] Progress update bytes=1006632960 percent=80

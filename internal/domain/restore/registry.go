...
// Registry manages restore provider factories
...
// RegisterFactory registers a restore provider factory
func (r *Registry) RegisterFactory(name string, factory ProviderFactory) error {
	r.mu.Lock()
	defer r.mu.Unlock()

	if _, exists := r.factories[name]; exists {
		return fmt.Errorf("restore provider factory %s already registered", name)
	}
...
func (r *Registry) Create(name string) (Provider, error) {
	...
	if !exists {
		return nil, fmt.Errorf("restore provider factory %s not found", name)
	}
...
// ProviderInfo contains information about a restore provider

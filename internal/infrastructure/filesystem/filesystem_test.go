...
// mockSizeEstimation sets up the mock for size estimation calls
func mockSizeEstimation(mockExecutor *MockCommandExecutor, ctx context.Context, namespace, pod, path string) {
	sizeCmd := []string{"kubectl", "exec", "-n", namespace, pod, "--", "du", "-sb", path}
	mockExecutor.On("Execute", ctx, sizeCmd).Return("1024\t"+path+"\n", nil).Maybe()
}

// setupMockExecutorForBackup prepares a mock executor with standard expectations for backup operations
func setupMockExecutorForBackup(mockExecutor *MockCommandExecutor, ctx context.Context, opts backup.Options) {
	// Always mock size estimation as it's called by executeInternal
	mockSizeEstimation(mockExecutor, ctx, opts.Namespace, opts.PodName, opts.SourcePath)
}
...
func TestFilesystemProvider_Execute(t *testing.T) {
	...
	// Mock size estimation (called by executeInternal)
